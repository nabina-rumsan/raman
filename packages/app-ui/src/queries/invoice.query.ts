...
export const useInvoiceList = (pagination: any, status?: string, user?: string, project?: string, category?: string): UseQueryResult<any, Error> => {
  const { queryClient, RsClient } = useRumsan<ApiClient>();
  const query = useQuery(
    {
      queryKey: ['invoice_list', pagination, status, user, project, category],
      queryFn: () =>
